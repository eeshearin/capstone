...
# Take a 5% sample of the dataset
sampler <- function(dataset) 
{ 
      return(dataset[as.logical(rbinom(length(dataset),1,0.05))]) 
} 
...
### N-gram Logic
# Create tokenizer control; a non-RWeka tokenizer was used due to irreconcilable RWeka errors
logic2 <- function(x) {unlist(lapply(ngrams(words(x), 2), paste, collapse = " "), use.names = FALSE)}
logic3 <- function(x) {unlist(lapply(ngrams(words(x), 3), paste, collapse = " "), use.names = FALSE)}

# Set in proper list format for control argument to tm function TermDocumentMatrix
...
# Process matrices into useful format (process is the same for each)
...
### Produce output file for use by server.R
